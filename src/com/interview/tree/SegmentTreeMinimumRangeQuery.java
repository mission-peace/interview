 ...
 * updateRange(int input[], int segment[], int lowRange, int highRange, int newVal) - updates all the values in given range with
 * given newVal.
 *
...
public class SegmentTreeMinimumRangeQuery {
    ...
    public void updateSegmentTreeRange(int input[], int segmentTree[], int startRange, int endRange, int newVal) {
        for(int i = startRange; i <= endRange; i++) {
            input[i] = newVal;
        }
        updateSegmentTreeRange(segmentTree, startRange, endRange, newVal, 0, input.length-1, 0);
    }

    private void updateSegmentTreeRange(int segmentTree[], int startRange, int endRange, int newVal, int low, int high, int pos) {
        ...
        updateSegmentTreeRange(segmentTree, startRange, endRange, newVal, low, middle, 2*pos + 1);
        updateSegmentTreeRange(segmentTree, startRange, endRange, newVal, middle+1, high, 2*pos + 2);
        segmentTree[pos] = Math.min(segmentTree[2*pos+1], segmentTree[2*pos+2]);
    }
    ...
    public static void main(String args[]){
        ...
        assert -2 == st.rangeMinimumQuery(segTree, 5, 6, input.length);
        assert -2 == st.rangeMinimumQuery(segTree, 0, 3, input.length);
    }
}
