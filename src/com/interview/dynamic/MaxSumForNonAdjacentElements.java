 ...
 * @author Tushar Roy
 *
 *         Find maximum sum for non adjacent elements. Variation is finding
 *         maximum sum non adjacent elements assuming its a circular array. So
 *         first element cannot be with last element.
 *
...
public class MaxSumForNonAdjacentElements {
	...
	/**
	 * Find maximum sum from left to right ignoring first element. Find maximum sum
	 * from right to left ignoring last element. Maximum of two will be the answer.
	 * It gurantees that both first and last element will be not selected together.
	 */
	...
	public static void main(String args[]) {
		...
		if (arr.length > 1) {
			...
			//For case where only 1 element is present in the array
			System.out.println(arr[0]);
		}
		int arr2[] = { -2, 1, 3, -4, 5 };
		// Result should be 8
		if (arr2.length > 1) {
			System.out.println(msn.maxSum(arr2));
			System.out.println(msn.maxSum(arr2, arr2.length - 1));
		} else {
			//For case where only 1 element is present in the array
