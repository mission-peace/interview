...
public class EditDistance {
    ...
    public int dynamicEditDistance(char[] str1, char[] str2){
        //extra row/col used to store edits if other string is null
        int temp[][] = new int[str1.length+1][str2.length+1];
        
        //initialize 0th row/col with natural numbers
        ...
        for(int i=1;i <=str1.length; i++){
            for(int j=1; j <= str2.length; j++){
                if(str1[i-1] == str2[j-1]){ //char is same, zero edit
                    temp[i][j] = temp[i-1][j-1]; //previous row && column
                }else{ //char is different, add 1 edit
                    temp[i][j] = 1 + min (
                        temp[i-1][j-1], //previous row && column
                        temp[i-1][j],   //previous row
                        temp[i][j-1] ); //previous column
                }
            }
        }
