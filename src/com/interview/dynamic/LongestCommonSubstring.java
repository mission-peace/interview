...
public class LongestCommonSubstring {
    ...
    public int longestCommonSubstring(char str1[], char str2[]){
        ...
        int max = 0;
        for(int i=1; i <= str1.length; i++){ // char index of string1
            for(int j=1; j <= str2.length; j++){ // char index of string2
                if(str1[i-1] == str2[j-1]){ // char match
                    T[i][j] = T[i-1][j-1] +1; // increment running substring
                    if(max < T[i][j]){
                        max = T[i][j];
                    }
                }
    ...
    public int longestCommonSubstringRec(char str1[], char str2[], int pos1, int pos2, boolean checkEqual){
        ...
        return Math.max(r1,Math.max(
            longestCommonSubstringRec(str1, str2, pos1-1, pos2, false), 
